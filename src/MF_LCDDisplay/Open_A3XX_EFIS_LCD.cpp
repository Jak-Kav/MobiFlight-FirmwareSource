...
void Open_A3XX_EFIS_LCD::showStd(int16_t state) {
  bool enabled;
...
// Show Values
void Open_A3XX_EFIS_LCD::showQNHValue(int16_t value) {
...
void Open_A3XX_EFIS_LCD::showQFEValue(int16_t value) {
  if (value > 9999) value = 9999;
  showQNHValue(value);
  setQFE(true);
}
...
void Open_A3XX_EFIS_LCD::handleMobiFlightCmd(char *cmd) {
  ...
  else if (strcmp(cmd, "setStd")==0) showStd((int16_t)data);
